function [MaxACC,index] = linearPerceptronExp(trainvector,testvector,epoch)
...
for T = epoch # should be done for 1 to 20
	...
	for digit = 0:9
		...
		Wd = [Wd perceptron(fname,T)]; # appending column vectors of Wd return for each digit
		# the vectors returned by perceptron are in the normalized form.	
	end
		printf("finished T = %d , columns for Wd : %d \n",T,columns(Wd));
	#printf(" Trained column Weight vector for each digit ");	
	#Wd
